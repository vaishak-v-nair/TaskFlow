...
function TaskCard({ task, userRole, userId, onUpdate, onDelete }: {
  task: Task;
  userRole: Role;
  userId: string;
  ...
  const nextStatus = canAdvanceStatus ? STATUS_FLOW[STATUS_FLOW.indexOf(task.status) + 1] : undefined;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            {task.assignee && <span className="text-xs text-zinc-500">-&gt; {task.assignee.name}</span>}
            {task.dueDate && (
...
export default function ProjectPage() {
  ...
  async function createTask() {
    ...
    const data = await fetchJson<Task>(`/api/projects/${id}/tasks`, {
      method: "POST",
    ...
    if (data.success) {
      setProject((current) => current ? { ...current, tasks: [data.data, ...current.tasks] } : current);
  ...
  async function addMember() {
    ...
    const data = await fetchJson<Member>(`/api/projects/${id}/members`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
    ...
    if (data.success) {
      setProject((current) => current ? { ...current, members: [...current.members, data.data] } : current);
  ...
  async function updateTask(taskId: string, updates: Partial<Task>) {
    ...
    const data = await fetchJson<Task>(`/api/tasks/${taskId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
    ...
    if (data.success) {
      setProject((current) => current ? {
        ...current,
        tasks: current.tasks.map((task) => task.id === taskId ? data.data : task),
      } : current);
    } else {
      setModalError(data.error);
    }
  ...
  async function deleteTask(taskId: string) {
    ...
    setModalError("");
    const data = await fetchJson<{ message: string }>(`/api/tasks/${taskId}`, {
      method: "DELETE",
      credentials: "include",
    });
  ...
  async function removeMember(uid: string) {
    ...
    setModalError("");
    const data = await fetchJson<{ message: string }>(`/api/projects/${id}/members`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
    ...
    if (data.success) {
      setProject((current) => current ? {
        ...current,
        members: current.members.filter((member) => member.userId !== uid),
      } : current);
    } else {
      setModalError(data.error);
    }
  ...
  async function deleteProject() {
    ...
    const data = await fetchJson<{ message: string }>(`/api/projects/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
    ...
    return (
      <div className="p-6 md:p-10 max-w-3xl">
        <div className="card p-6 text-center">
          <p className="text-red-400 text-sm">{pageError || "Unable to load this project."}</p>
        </div>
      </div>
    );
  }

  const filteredTasks = filter === "ALL" ? project.tasks : project.tasks.filter((task) => task.status === filter);
  const taskCounts = { ALL: project.tasks.length, TODO: 0, IN_PROGRESS: 0, DONE: 0 };
  project.tasks.forEach((task) => taskCounts[task.status]++);
            ...
            {(["ALL", "TODO", "IN_PROGRESS", "DONE"] as const).map((status) => (
              <button
                ...
                className={clsx(
                  ...
                  filter === status ? "bg-indigo-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"
                )}
              >
                {status === "ALL" ? "All" : STATUS_LABEL[status]} ({taskCounts[status]})
              </button>
            ))}
          </div>
          ...
          {project.members.map((member) => (
            <div key={member.userId} className="card p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-zinc-200 font-medium text-sm">{member.user.name}</p>
                <p className="text-zinc-500 text-xs">{member.user.email}</p>
              </div>
              <span className={clsx("badge", member.role === "ADMIN" ? "badge-in-progress" : "badge-todo")}>
                {member.role}
              </span>
              {userRole === "ADMIN" && member.userId !== userId && (
                <button onClick={() => removeMember(member.userId)} className="text-zinc-600 hover:text-red-400 transition-colors">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="card w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="input" placeholder="e.g. Design landing page" value={taskForm.title} onChange={(e) => setTaskForm({ ...taskForm, title: e.target.value })} />
              </div>
              <div>
          ...
          <div className="card w-full max-w-sm p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="email" className="input" placeholder="member@example.com" value={memberEmail} onChange={(e) => setMemberEmail(e.target.value)} />
              </div>
              <div>
                <label className="label">Role</label>
                <select className="input" value={memberRole} onChange={(e) => setMemberRole(e.target.value as Role)}>
